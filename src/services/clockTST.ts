...
export class TSTDate {
     ...
     */
    constructor (milliseconds?: number, cookie = true) {
    ...
    toJSON() {
        ...
        let out = date.join('-').concat('T', time.join(':'), '.', tst.millisecond.toString(), 'Z');
        return out;
    }
    ...
    setGameStart(milliseconds: number) {
        ...
        this.settings.tst = this.init(this.settings);
    }
    ...
    private createTST(timeSinceUpdate: number, ratio = 1) {
        ...
        while (timeSinceUpdate - game.calendar[month - 1] * lengthDay >= 0) {
            timeSinceUpdate -= game.calendar[ - 1] * lengthDay;
            month ++;
        }
        ...
        let milliseconds = timeSinceUpdate;
     ...
     * @param settings saved or newly created settings
     * @returns {ITst}
     */
    private init(settings: ISettings): ITst {
        /**
